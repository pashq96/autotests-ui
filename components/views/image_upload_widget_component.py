...
class ImageUploadWidgetComponent(BaseComponent):
    def __init__(self, page: Page):
        ...
        self.image_upload_info_icon = Icon(page, '{identifier}-image-upload-widget-info-icon', 'Image icon')
    ...
    def check_visible(self, is_image_uploaded: bool = False):
        self.image_upload_info_icon.check_visible(identifier='create-course-preview')

        self.image_upload_info_title.check_visible(identifier='create-course-preview')
        self.image_upload_info_title.check_have_text(
            'Tap on "Upload image" button to select file',
            identifier='create-course-preview'
        )

        self.image_upload_info_description.check_visible(identifier='create-course-preview')
        self.image_upload_info_description.check_have_text(
            'Recommended file size 540X300',
            identifier='create-course-preview'
        )

        self.upload_button.check_visible(identifier='create-course-preview')

        if is_image_uploaded:
            self.remove_button.check_visible(identifier='create-course-preview')
            self.preview_image.check_visible(identifier='create-course-preview')
